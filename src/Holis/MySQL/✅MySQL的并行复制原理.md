...
[✅介绍下MySQL 5.6中的组提交](https://www.yuque.com/hollis666/axzrte/bb860tpuha0cuza2?view=doc_embed)
...
WriteSet 是通过检测两个事务是否更新了相同的记录来判断事务能否并行回放的，因此需要在运行时保存已经提交的事务信息以记录历史事务更新了哪些行，并且在做更新的时候进行冲突检测，拿新更新的记录计算出来的hash值和retset作比较，如果不存在，那么就认为是不冲突的，这样就可以共用同一个last_committed 、
...
<!--page footer-->
- 原文: <https://www.yuque.com/hollis666/axzrte/igarxy867n7bgq1q>